...
import { fetchKQL, getNavStructure, getPage } from '../lib/api'
...
export default function Index({ navigation, page, featuredVideos, preview }) {
  ...
  return (
    <Layout preview={preview} title="Christliche Apologetik">
      <MobileMenu nav={navigation} />
      <Hero heading={page.content.hero} image={page.content.image[0].url}>
        <Navigation nav={navigation} background="dark" />
      </Hero>
      <section className="max-w-5xl z-10 relative px-3 md:px-0 mx-auto -mt-12 mb-28 transform md:scale-90">
        <div className="flex flex-col md:flex-row space-y-2">
          <div className="w-full hover:-translate-y-1 duration-300 ease-out transition-transform hover:cursor-pointer md:w-1/3 md:transform md:scale-110 md:-translate-x-4">
            <VideoThumbnail data={page.videos[0]}  />
          </div>
          <div className="w-full md:w-1/3 md:transform md:scale-110  md:-rotate-3 md:translate-y-1 duration-300 ease-out hover:-translate-y-0.5 transition-transform hover:cursor-pointer">
            <VideoThumbnail data={page.videos[1]} />
          </div>
          <div className="w-full md:w-1/3 md:transform md:scale-110 md:translate-x-4 md:mt-0 md:-translate-y-1  md:rotate-3 hover:-translate-y-2 transition-transform duration-300 ease-out hover:cursor-pointer">
      ...
      <main className="max-w-2xl mx-auto px-4 pb-20 text-gray-900 break-words">
        {page.content.block.map((block, index)=>{
          ...
          if (block.type == 'heading') {
            ...
            return (<div className="prose md:prose-base my-2"><Heading><span dangerouslySetInnerHTML={{__html: block.content.text}}></span></Heading></div>)
          }
          if (block.type == 'testimonials') {
            ...
            const quotes = block.content.testimonials.map((quote, index)=>(
              <div key={index} className="flex my-2 text-sm">
                <svg className={`${colors[index]} text-opacity-20 flex-shrink-0`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 350 350" width="35" height="35">
            ...
            return (
              <div className="h-72 overflow-hidden relative mt-4 -mx-4 md:-mx-6 bg-gray-400 bg-opacity-10 md:bg-opacity-0">
                <div className="h-16 bg-gradient-to-b from-gray-100 to-transparent absolute w-full z-10"></div>
  ...
  return (
    <a href="#" className="h-0 block bg-black relative pb-16:9 w-full overflow-hidden shadow-sm">
      <div className="group absolute w-full h-full top-0 right-0 grid stack" >
        ...
        <div className="absolute bottom-0 left-0 p-4 md:p-3 text-white text-3xl md:text-2xl font-extrabold leading-none md:tracking-wide" style={{filter: "drop-shadow(0 0 20px rgb(10 40 40 / 80%))"}}>
