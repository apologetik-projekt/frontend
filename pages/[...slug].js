      ...
      <main className="max-w-4xl mx-auto px-4 pt-4 pb-10">
        {page.content.blocks.map((block, index)=>{
          ...
          if (block.type == 'heading') {
            ...
            return (<div key={index} className="prose max-w-2xl mx-auto md:prose-base"><Heading><span dangerouslySetInnerHTML={{__html: block.content.text}}></span></Heading></div>)
          }
          if (block.type === "team") {
            const members = block.content.members.map((member)=>(
                <div key={index} className="w-full md:w-1/2 md:px-4 mt-2 mb-8">
