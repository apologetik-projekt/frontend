...
export default function Hero({children, heading}){
	const variants = {
		before: {
			x: '40%',
			y: -20,
	...
	return (
		<header className="min-h-[90vh] md:min-h-[50vh] max-h-[100vh] w-full text-white overflow-hidden noise relative grid stack items-end">
        <div className="radial-gradient min-h-[50vh] md:min-h-[50vh] max-h-[100vh] w-full h-full pb-24 mix-blend-hard-light isolate items-stretch transition-colors">
					{children}
          ...
          <section className="flex flex-col md:flex-row-reverse justify-between space-x-2 md:pb-4">
            <motion.div variants={variants} initial="before" animate="after" className="my-2 md:mt-0 md:w-3/5">
              <Image alt="Zwei Personen unterhalten sich" objectFit="cover" width="570" className="m-0 rounded" height="360" loading="eager" src={"/conversation.jpg"} />
            </motion.div>
            <motion.div variants={textVariant} initial="hidden" animate="show" className="-mt-6 sm:mt-8 md:w-2/5 md:min-w-[350px]">
				...
				<svg id="bg-slash" className="z-10" preserveAspectRatio="none" width="100%" height="60px" viewBox="0 0 1000 50">
					<path width="100%" height="100%" d="M 0 50 L 1000 50 L 1000 20 L 0 50 Z" fill="#F5F5F5" stroke="none"/>
				</svg>
